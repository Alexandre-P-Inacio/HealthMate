...
const MedicalDiaryScreen = ({ navigation }) => {
  ...
  const [missedMedications, setMissedMedications] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchConfirmedMedications(selectedDate); // Added function call to fetch confirmed medications
    scheduleNextMedicationCheck();
    checkMissedMedications();
  }, [selectedDate, userId]);

  // Verifica o próximo medicamento a cada minuto
  useEffect(() => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      console.log('MedicalDiaryScreen focused - refreshing data');
      if (userId) {
  ...
  const loadUserData = async () => {
    try {
      // Get user data from DataUser (adjust based on your app's user management)
      const userData = DataUser.getUserData();
      if (userData && userData.id) {
        setUserId(userData.id);
        console.log(`Loaded user ID: ${userData.id}`);
      } else {
        console.log('User data not found or missing ID');
      }
    } catch (error) {
  ...
  const checkNextMedication = async () => {
    try {
      const storedMeds = await AsyncStorage.getItem(MEDS_STORAGE_KEY);
      if (!storedMeds) return;
      ...
      // Filtrar apenas medicamentos futuros
      const upcomingMeds = medications.filter(med => {
        const medTime = new Date(med.nextDose);
        return medTime > now;
      });
      
      // Ordenar por horário (mais próximo primeiro)
      upcomingMeds.sort((a, b) => {
        return new Date(a.nextDose) - new Date(b.nextDose);
      });
      
      if (upcomingMeds.length > 0) {
        setNextMedication(upcomingMeds[0]);
      } else {
        setNextMedication(null);
      }
    } catch (error) {
      console.error('Erro ao verificar próximos medicamentos:', error);
    }
  };

  const scheduleNextMedicationCheck = async () => {
    try {
      ...
      // Agendar notificações para os próximos medicamentos
      for (const med of medications) {
        ...
        if (medTime > now) {
          ...
          if (timeUntilMed > 30 * 60 * 1000) {
            await Notifications.scheduleNotificationAsync({
              content: {
                title: 'Lembrete de Medicamento',
                body: `Você precisará tomar ${med.name} em 30 minutos`,
              },
              trigger: { seconds: (timeUntilMed - 30 * 60 * 1000) / 1000 },
            });
          }
          ...
          await Notifications.scheduleNotificationAsync({
            ...
            trigger: { seconds: timeUntilMed / 1000 },
          });
        }
      }
    } catch (error) {
  ...
  const fetchEntriesByDate = async (date) => {
    try {
      ...
      // Filter entries by date AND user ID
  ...
  const checkMissedMedications = async () => {
    try {
      const storedMeds = await AsyncStorage.getItem(MEDS_STORAGE_KEY);
      if (!storedMeds) return false;
      
      const medications = JSON.parse(storedMeds);
      const now = new Date();
      
      // Verificar se há medicamentos perdidos
      const missed = medications.some(med => {
        const medTime = new Date(med.nextDose);
        return medTime < now;
      });
      ...
      console.error('Erro ao verificar medicamentos perdidos:', error);
      return false;
    }
  };
  ...
  const fetchConfirmedMedications = async (date) => {
    try {
      ...
      if (!userId) {
        console.log('No user ID available, cannot fetch confirmed medications');
      ...
      console.log(`Fetching confirmed medications for date: ${formattedDate} and user: ${userId}`);
      ...
      if (confirmError) {
        ...
        console.log(`Found ${filteredData.length} confirmed medications (old table)`);
        ...
        console.log(`Found ${processedData.length} confirmed medications (new table)`);
  ...
  // Renderizar componente de próximo medicamento
  const renderNextMedication = () => {
    if (!nextMedication) return null;
    
    const nextDoseTime = new Date(nextMedication.nextDose);
    const now = new Date();
    ...
    let timeText = "";
    if (diffHours > 0) {
      timeText = `${diffHours}h ${diffMinutes}min`;
    } else {
      timeText = `${diffMinutes} minutes`;
    }
      ...
      <View style={styles.nextMedContainer}>
        <View style={styles.nextMedContent}>
          ...
          <View style={styles.nextMedInfo}>
            <Text style={styles.nextMedTitle}>Next Medication</Text>
            <Text style={styles.nextMedName}>{nextMedication.name} ({nextMedication.dosage})</Text>
            <Text style={styles.nextMedTime}>In {timeText}</Text>
          </View>
        </View>
      </View>
    );
  ...
  const handleSaveEntry = async () => {
    try {
      ...
      if (editingEntry) {
        ...
        const updatedEntries = allEntries.map(entry => {
          if (entry.id === editingEntry.id) {
            return {
              ...
              // Keep the original user_id if it exists
        ...
        const updatedEntries = [...allEntries, newEntryData];
        await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(updatedEntries));
      }
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          >
            ...
            {missedMedications && <View style={styles.notificationBadge} />}
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  medicationItemPastDue: {
    borderColor: '#FFE0E0',
    borderWidth: 1,
    backgroundColor: '#FFF5F5',
  },
  medicationIconPastDue: {
    backgroundColor: '#FF4842',
  },
  medicationItemUpcoming: {
    borderColor: '#E0F7FF',
    borderWidth: 1,
    backgroundColor: '#F5FCFF',
  },
  medicationIconUpcoming: {
  ...
  medicationItemMissed: {
    borderColor: '#FFE0E0',
    borderWidth: 1,
    backgroundColor: '#FFF0F0',
  },
  medicationIconMissed: {
    backgroundColor: '#FF4842',
  },
  takenText: {
    color: '#4CAF50',
